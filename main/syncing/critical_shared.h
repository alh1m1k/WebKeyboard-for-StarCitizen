 ...
 * With difference that this barrier implements a shared_lock interface.
 * It allows an arbitrary number of concurrent reads but synchronizes writes and writes with reads.
 * The time spent in the lock should be minimal, no IO operations (the task was active when waiting)
 * Use this class only to synchronize reading and writing inside ram memory.
 * Incorrect use of the class may result in a device reboot.
 */
