...
namespace http::socket {
	...
	asyncSocket::asyncSocket(httpd_handle_t hd, int fd) noexcept : hd(hd), fd(fd) {};

	bool asyncSocket::valid() const noexcept {
		return (hd != nullptr && fd != 0) && httpd_ws_get_fd_info(hd, fd) == HTTPD_WS_CLIENT_WEBSOCKET;
	}

	resBool asyncSocket::write(const uint8_t* buffer, size_t size, httpd_ws_type_t type) noexcept {
		#if defined(SYNC_WRITE_OP_WITH_MUTEX) 
	...
	resBool asyncSocket::write(const message& msg, httpd_ws_type_t type) noexcept {
		debugIf(LOG_SOCKET, "socket::write", msg.size());
		return write((const uint8_t*)msg.data(), msg.size(), type);
	}
	
	resBool asyncSocket::write(const char* msg, httpd_ws_type_t type) noexcept {
